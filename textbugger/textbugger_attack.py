...
class TextBugger(object):
    def __init__(self, classifier, word2vec_model_path):
        self.nlp = spacy.load("en")
        self.classifier = classifier
        self.topn = 5
        self.sim_epsilon = 0.8
        self.sentence_encoder = hub.Module(os.path.join(PROJECT_ROOT,SENTENCE_ENCODER))
        self.word2vec = gensim.models.KeyedVectors.load_word2vec_format(word2vec_model_path, binary=False)
    ...
    def similarity(self,x1, x2):
        ...
        x1_x2_tensor = self.sentence_encoder([" ".join(x1), " ".join(x2)])
        with tf.Session() as tf_sess:
            tf_sess.run([tf.global_variables_initializer(), tf.tables_initializer()])
            x1_x2 = tf_sess.run(x1_x2_tensor)
