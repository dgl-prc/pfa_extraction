...
def get_next_states(curent_state, trigger_word, trans):
    possibe_next = []
...
def get_prediction(pfa, persistence, trans, action_trace_paths, t_word_traces_path=None, t_prdct_grnd_path=None):
    #########
    # predict
    #########
    terminate_count = 0
...
def get_pfa_acc_v2(pfa, used_traces_path, persistence, t_word_traces_path=None, t_prdct_grnd_path=None):
    ...
    for action_trace_path in action_trace_paths:
        ...
        for i, start_state in enumerate(state_transition):
            ...
            if i + 1 == len(state_transition) - 1:
                break
    return get_prediction(pfa, persistence, trans, action_trace_paths)
